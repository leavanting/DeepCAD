...
from data_process import shuffle_datasets, train_preprocess, test_preprocess, test_preprocess_lessMemory, test_preprocess_lessMemoryNoTail_chooseOne
...
##############################################################################################################################################################
# network architecture GPU access
denoise_generator = Network_3D_Unet(in_channels = 1,
                                    out_channels = 1,
                                    final_sigmoid = True)
if torch.cuda.is_available():
    print('\033[1;31mUsing GPU for processing...\033[0m')
    ...
    if '.pth' in aaa:
        ...
        for N in range(len(img_list)):
            ...
            for index in range(len(name_list)):
                ...
                time_left = batches_left * (time.time() - prev_time)# datetime.timedelta(seconds=batches_left * (time.time() - prev_time))
                prev_time = time.time()
                prev_time = time.time()
                ################################################################################################################
                if index%1 == 0:
                    ...
                    print('\r[Model %d/%d, %s] [Stack %d/%d, %s] [Patch %d/%d] [Time Cost: %.0d s] [Time Left: %.0d s]     ' 
                    % (
                        pth_index+1,
                        len(model_list),
                        pth_name,
